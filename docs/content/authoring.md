...
创建 `content/posts/my-entry/zh.md`，目录名使用小写英文、数字和连字符。目录标识组成永久链接（中文 `/posts/my-entry/`，英文 `/en/posts/my-entry/`），同一目录下的 `en.md` 是对应英文版本。也可以直接运行：

```powershell
pnpm new-post my-entry   # 生成 zh.md 与 en.md 草稿
```
...
标题、日期、用途（`kind`）、展示形式（`layout`）及至少一个标签必填。日期显示统一使用 UTC 日历日期，避免不同构建环境出现日期偏移。修改文章目录名会改变 URL，应同时处理旧链接。
...
2. 准备发布时将 `draft` 改为 `false` 或删除该字段。
3. 执行 `pnpm content:check`；错误会指出文件和原因。
4. 执行 `pnpm build`，再执行 `pnpm preview --port 4330` 检查真实产物与导航栏搜索。
